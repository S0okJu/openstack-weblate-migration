...
function finish {
    ...
    # Skip subunit generation if testr environment is not available
    if [ -f "$TESTR_VENV/bin/generate-subunit" ]; then
        ...
        echo "Warning: testr environment not found, skipping subunit generation"
    fi
...
EOF
        ...
        if [[ "${PREV_PATCH_ID}" != "${CUR_PATCH_ID}" ]]; then
            ...
            git status
            # DRY RUN: Commented out git review to avoid pushing to Gerrit
            # set +e
...
# Pull all translations from Weblate for each component in the project.
# It downloads all translations for each component as a zip archive, extracts them,
# and then removes new files that are not translated enough.
function pull_from_weblate {
    local project=$1
    local weblate_url=${WEBLATE_URL:-"https://openstack.weblate.cloud"}
    local token_header="Authorization: Token $WEBLATE_TOKEN"

    # Get all components for the project
    components=$(curl -s -H "$token_header" "$weblate_url/api/projects/$project/components/" | \
        python3 -c "import sys, json; [print(comp['slug']) for comp in json.load(sys.stdin).get('results', [])]")

    for component in $components; do
        echo "Pulling translations for component: $component"
        zipfile="translations-$component.zip"
        # Download all translations for this component as a zip archive
        curl -s -H "$token_header" \
            "$weblate_url/api/components/$project/$component/file/?format=zip" \
            -o "$zipfile"
        if [ ! -s "$zipfile" ]; then
            echo "[WARN] Could not download translations for $component"
            rm -f "$zipfile"
            continue
        fi
