...
# Weblate: Upload .pot files as new source strings for each component
for m in $ALL_MODULES; do
    pot_files=$(find "$m" -type f -name "*.pot" 2>/dev/null || echo "")
    # For each component, upload pot
    for pot in $pot_files; do
        pot_filename=$(basename "$pot" .pot)
        module_path=$(dirname "$pot")
        branch_slug=$(echo "$ZANATA_VERSION" | tr '.' '-')
        module_slug=$(echo "$module_path" | sed 's/[\/.]/-/g')
        component_slug="${branch_slug}-${module_slug}-$(echo "$pot_filename" | sed 's/[^a-zA-Z0-9_-]/-/g')"
        upload_pot_to_component "$PROJECT" "$pot_filename" "$pot" "" "$module_path" "$ZANATA_VERSION" "$component_slug"
    done
done

# Move pot files to translation-source directory
