...
from ZanataUtils import ProjectConfig


def get_args():
    parser = argparse.ArgumentParser(
        description='Generate a zanata.xml file for this project '
        'so we can process translations')
    ...
    parser.add_argument(
        '-r', '--rule', nargs=2, metavar=('PATTERN', 'RULE'),
        action='append',
        help='Append a rule, used by the Zanata client to match .pot files '
             'to translations. Can be specified multiple times, and if no '
             'rules are specified a default will be used.')
    parser.add_argument(
...
def main():
    ...
    try:
        ...
        ProjectConfig(
            zc, args.file, rules, args.verify,
